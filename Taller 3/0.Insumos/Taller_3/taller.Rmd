...
```{r}
train <- read.csv('train_join.csv')
test <-  read.csv('test_join.csv')
...
El objetivo de este articulo es predecir el precio de las viviendas en la ciudad de bogota. Esta variable esta medida a traves de 3 anos distintos, lo que en terminos reales no representa lo mismo, por esto en primer lugar el proceso que se realizo fue una deflactacion de estas variables en terminos reales de tal forma que esto no afectara el proceso de modelacion. con esto en cuenta tenemos que el precio de entrada para comprar vivienda en Bogota tomando como referencia esta base de datos  es de 300 millones y el maximo es de 1650 millones y el promedio del precio de la Vivienda es de aproximadamente 650 millones. Bogota en terminos de precios de vivienda se encuentra por debajo del promedio nacional, siguiendo el Indice de precios de Vivienda del DANE, el indice de precios es de 126 puntos mientras que el promedio nacional es de 130, y el aumento de precios entre estos 3 anos es moderado donde en promedio el precio se ha incrementado un 8 %.
...
```
```{r}
format(summary(train$city),scientific=FALSE)
```
