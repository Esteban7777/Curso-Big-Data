...
```{r}
table(train_hogares$educacion_jefe)
```
...
cualitativas <- c("sexo_jefe", "regimen_subsidiado_jefe", "educacion_jefe", "desempleo_jefe", "posicion_jefe","Personas_habitacion_r")
...
# Iterar sobre las variables cualitativas y crear un boxplot para cada una
...
# Combinar los gráficos en una grilla de 3 filas y 2 columnas
...
Tambien se observa como a medida que sube el numero de perosnas por habitacion la media del ingreso del hogar disminuye, tal como se menciona en la revision de literatrura
...
```{r}
train_hogares %>%
  ggplot(aes(x=tipo_casa,y=log(Ingtotugarr))) +
  geom_boxplot()
```
...
train_hogares<-train_hogares %>%
  mutate(edad_jefe_joven = ifelse(edad_jefe<=28,1,0))
```
...
```{r}
train_hogares %>%
  ggplot(aes(x=edad_jefe_joven,y=log(Ingtotugarr))) +
  geom_boxplot()
```


```{r}
table(train_hogares$subsidio_jefe)
```
