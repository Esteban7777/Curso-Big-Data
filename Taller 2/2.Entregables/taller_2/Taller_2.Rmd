...
```{r}
boxplots <- lapply(cualitativas, function(var) {
  train_hogares %>%
    ggplot(aes(x = factor({{ var }}), y = Ingtotug, fill = factor({{ var }}))) +
    geom_boxplot() +
    labs(title = paste("Boxplot de", var, "vs. Ingreso Total"),
         x = var,
         y = "Ingreso Total") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Mostrar los boxplots individualmente
