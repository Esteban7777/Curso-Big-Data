...
predictores<-c("informalidad_jefe","subsidio_jefe","edad_jefe",
                      "posicion_jefe","Ina_jefe","Des_jefe","Oc_jefe","Pet_jefe","ocupacion_jefe","educacion_jefe","regimen_jefe","sexo_jefe","Clase","Dominio","P5000","P5010","P5090","P5100","P5130","P5140","Nper","Npersug","Depto","regimen_subsidiado_jefe","desempleo_jefe","Personas_habitacion","tipo_casa","edad_jefe_joven","Personas_habitacion_round")
```
...
Se entrena el siguiente modelo

```{r}
logit<-glm(as.formula(
  paste("Pobre~",
        paste(predictores, collapse = " + "))),data = train_hogares)

summary(logit)
```

Se identifican las variables significativas y se retiran las que
producen multicolinealidad

```{r}
predictores_logit<-c("desempleo_jefe", #Completa
...
Se entrena el modelo con estas variables
...
confusionMatrix(data = train_hogares$logit_predict,
                reference = train_hogares$Pobre,
                positive="Pobre",
                mode = "prec_recall")
```

Se evalua fuera de muestra y se genera el submission para Kaggle
...
También se genera un modeo con las interacciones entre las variables.
...
train_hogares$logit2_predict<-ifelse(train_hogares$prob_logit2>=0.5,1,0)
confusionMatrix(data = train_hogares$logit_predict2,
                reference = train_hogares$Pobre,
                positive="Pobre",
                mode = "prec_recall")
```

Se realizan las predicciones fuera de muestra

```{r}
test_hogares$prob_logit2<-predict(object = logit2,newdata = test_hogares)
test_hogares$logit2_predict<-ifelse(test_hogares$prob_logit2>0.5,1,0)
table(test_hogares$m6_predict)
```

Se genera el submision para Kaggle
...
##Predicciones con Árbol Para entrenar el árbol se utilizó la siguiente
sintaxis
...
##Predicciones con Ramdom Forest Se realiza un entrenamiento inicial de
un modelo de Ramdom Forest utilizando la siguiente sintaxis
...
Después se prueba aumentar la cantidad de arboles para identificar que
...
Finalmente probamos aumentar la cantidad de variables por arbol.
...
el formato adecuado para realizar submitir en Keaggle.
...
##Predicciones con Regularización. Para aplicar regularización tenemos
que trabajar con nuestros datos en formato matriz
...
##Predicciones con Árbol. Para realizar la predicción con arboles de
decisión se realizó la siguiente sintaxis
